...
public class GuiVault2by2Settings extends GuiScreen
{
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        {
            this.fontRenderer.drawString(this.tile.getOthers().get(k).toString(), this.width / 2 - 70, this.height / 2 - 52 + k * 20, 0);
        }
    ...
    private void addPlayerToTileEntity()
    {
        ...
        if (entityplayer != null)
        {
            ...
            this.tile.setOthers(s1);
            this.tile.addToMax();
            PacketsRegistery.network.sendToServer(new PacketListNBT(s1, this.tile.getPos().getX(), this.tile.getPos().getY(), this.tile.getPos().getZ(), true, "add", 0));
        }
    }
    ...
    protected void actionPerformed(GuiButton button) throws IOException
    {
        ...
        {
            case 0:
                this.tile.removeToMax();
                this.tile.getOthers().remove(0);
                PacketsRegistery.network.sendToServer(new PacketListNBT("", this.tile.getPos().getX(), this.tile.getPos().getY(), this.tile.getPos().getZ(), true, "remove", 0));
                break;

            case 1:
                this.tile.removeToMax();
                this.tile.getOthers().remove(1);
                PacketsRegistery.network.sendToServer(new PacketListNBT("", this.tile.getPos().getX(), this.tile.getPos().getY(), this.tile.getPos().getZ(), true, "remove", 1));
                break;

            case 2:
                this.tile.removeToMax();
                this.tile.getOthers().remove(2);
                PacketsRegistery.network.sendToServer(new PacketListNBT("", this.tile.getPos().getX(), this.tile.getPos().getY(), this.tile.getPos().getZ(), true, "remove", 2));
                break;

            case 3:
                this.tile.removeToMax();
                this.tile.getOthers().remove(3);
                PacketsRegistery.network.sendToServer(new PacketListNBT("", this.tile.getPos().getX(), this.tile.getPos().getY(), this.tile.getPos().getZ(), true, "remove", 3));
                break;

            case 4:
                this.tile.removeToMax();
                this.tile.getOthers().remove(4);
                PacketsRegistery.network.sendToServer(new PacketListNBT("", this.tile.getPos().getX(), this.tile.getPos().getY(), this.tile.getPos().getZ(), true, "remove", 4));
        }
    }
